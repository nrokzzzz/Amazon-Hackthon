...
// returns ONE of:
//   { file: { filename, mimeType, data } }  -> base64; pass straight to Gemini
//                                              (PDFs + images: Gemini reads/OCRs these natively)
//   { text: '...' }                          -> extracted text to append to the prompt
//                                              (docx, xlsx/xls, csv, txt, html)
//   { skipped: true, reason }                -> unsupported or too large
//
// This is shared by the Gmail webhook and the manual upload endpoint so every
// path reads timetables / exam schedules / holiday lists the same way.

export const MAX_FILE_BYTES = 15 * 1024 * 1024; // keep Gemini requests reasonable

// Gemini-native inline types — sent as files, not text.
...
export async function extractAttachment(buffer, filename = 'attachment', declaredMime = '') {
  ...
  // 1) PDFs + images -> hand to Gemini natively (best for timetable layouts/scans).
