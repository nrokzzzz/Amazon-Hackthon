...
// connected student whose watch is missing or about to expire. No user action.
...
export async function reconcileWatches(now = Date.now()) {
  const students = await Student.find({ 'gcal.refresh_token': { $exists: true, $ne: null } });
  ...
  for (const s of students) {
    const watching = Boolean(s.gmail?.watching);
    ...
    try {
      ...
      failed++;
      console.error(`[gmail] watch reconcile failed for ${s.gcal?.email || s._id}:`, err?.message || err);
    }
  }
